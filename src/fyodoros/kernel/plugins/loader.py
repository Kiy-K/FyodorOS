...
from the `~/.fyodor/plugins/` directory.
"""
...
class PluginLoader:
    ...
    def __init__(self, kernel=None):
        self.kernel = kernel
        self.plugins_dir = Path.home() / ".fyodor" / "plugins"
        self.loaded_plugins = {}
    ...
    def _load_plugin(self, plugin_dir: Path, manifest_path: Path, agent):
        ...
        spec = importlib.util.spec_from_file_location(f"fyodor_plugin_{name}", module_path)
        module = importlib.util.module_from_spec(spec)
        sys.modules[f"fyodor_plugin_{name}"] = module
