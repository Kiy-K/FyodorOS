...
/// `python3 -m fyodoros serve` to launch the kernel in API mode.
///
/// # Returns
///
...
fn start_kernel() -> String {
    ...
    // For this prototype, we'll try to execute `python3 -m fyodoros serve` assuming we are in repo root
    // or set PYTHONPATH.
